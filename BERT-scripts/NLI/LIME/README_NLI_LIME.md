# NLI LIME Explanation and Jaccard Similarity Workflow

This folder contains anonymized scripts for running LIME explanations on five BERT-family models fine-tuned for the SNLI/NLI task and for computing pairwise Jaccard similarity between the important tokens selected by different models.
...
```bash
pip install torch transformers pandas numpy tqdm lime scikit-learn
```
...
Avoid using public documentation with absolute private paths such as:
...
Use neutral relative paths instead:
...
## 5. Run LIME for All Five Models

```bash
python3 run_bert_nli_lime_all_5_models_fast.py \
  --snli_test_file ./snli_1.0/snli_1.0_test.txt \
  --checkpoint_root ./outputs/snli_nli_models \
  --output_dir ./outputs/nli_lime \
  --num_runs 3 \
  --max_len 256 \
...
```text
outputs/nli_lime/
├── bert_base/run_01/bert_base_nli_lime_run_01.csv
├── distilbert/run_01/distilbert_nli_lime_run_01.csv
├── bert_medium/run_01/bert_medium_nli_lime_run_01.csv
├── bert_mini/run_01/bert_mini_nli_lime_run_01.csv
└── bert_tiny/run_01/bert_tiny_nli_lime_run_01.csv
```
...
python3 run_bert_nli_lime_all_5_models_fast.py \
  --snli_test_file ./snli_1.0/snli_1.0_test.txt \
  --checkpoint_root ./outputs/snli_nli_models \
  --output_dir ./outputs/nli_lime_debug \
  --num_runs 1 \
  --limit_samples 20 \
...
## 6. Run LIME for Selected Models
...
## 8. Compute LIME-Based Jaccard Similarity Matrix

After generating the LIME CSV files, compute pairwise Jaccard similarity using the top-K important tokens.

```bash
python3 create_nli_lime_jaccard_table.py \
  --input_root ./outputs/nli_lime \
  --output_dir ./outputs/nli_lime_jaccard_tables \
  --num_runs 3 \
  --k 10 \
  --label_source true
```
...
python3 create_nli_lime_jaccard_table.py \
  --input_root ./outputs/nli_lime \
  --output_dir ./outputs/nli_lime_jaccard_tables_predicted \
  --num_runs 3 \
  --k 10 \
...
```text
outputs/nli_lime_jaccard_tables/
├── NLI_LIME_jaccard_run_1.csv
├── NLI_LIME_jaccard_run_2.csv
├── NLI_LIME_jaccard_run_3.csv
├── NLI_LIME_jaccard_all_runs_long.csv
├── NLI_LIME_jaccard_summary_mean_variance.csv
├── NLI_LIME_jaccard_summary_mean_variance_rounded.csv
└── NLI_LIME_jaccard_summary_latex.txt
```
...
| File | Purpose |
|---|---|
| `run_bert_nli_lime_all_5_models_fast.py` | Runs LIME explanations for all or selected NLI models |
| `create_nli_lime_jaccard_table.py` | Computes pairwise Jaccard similarity from LIME outputs |
| `README_NLI_LIME.md` | Instructions for running the LIME workflow |
