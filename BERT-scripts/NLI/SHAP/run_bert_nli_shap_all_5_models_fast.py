...
# -*- coding: utf-8 -*-
"""
Run SHAP explanations for five BERT-family NLI models.
...
def custom_shap_tokenizer(text: str, return_offsets_mapping: bool = True):
    """Whitespace tokenizer compatible with shap.maskers.Text."""
    text = str(text)
    pos = 0
    offset_ranges = []
    input_ids = []
    for match in re.finditer(r"\s", text):
        ...
        if start > pos:
            ...
            input_ids.append(text[pos:start])
        pos = end
    if pos < len(text):
        offset_ranges.append((pos, len(text)))
        input_ids.append(text[pos:])
...
def run_shap_for_model_run(
    ...
    spec = MODEL_SPECS[model_key]
    out_path = output_dir / model_key / f"run_{run_id:02d}" / f"{model_key}_nli_shap_run_{run_id:02d}.csv"
    out_path.parent.mkdir(parents=True, exist_ok=True)

    set_seed(seed)
    masker = shap.maskers.Text(custom_shap_tokenizer)
    explainer = shap.Explainer(predict_proba, masker, output_names=CLASS_NAMES)

    rows = []
    begin = time.time()

    for _, row in tqdm(dx.iterrows(), total=len(dx), desc=f"SHAP {model_key} run {run_id:02d}"):
        text = row["sent"]
        probs = predict_proba([text])[0]
        shap_values = explainer([text]) if not max_evals else explainer([text], max_evals=max_evals)

        per_class = {"contradiction": [], "neutral": [], "entailment": []}
        for token_item in shap_values[0]:
            token_data = str(token_item[0].data)
            values = np.array(token_item.values).reshape(-1)
            base_values = np.array(token_item.base_values).reshape(-1)

            for class_idx, class_name in enumerate(CLASS_NAMES):
                val = float(values[class_idx]) if class_idx < len(values) else 0.0
                base = float(base_values[class_idx]) if class_idx < len(base_values) else 0.0
                per_class[class_name].append((token_data, val, base))
        ...
        rows.append(
            {
                ...
                "sent2": row["sent2"],
                "true_out": row["label"],
                "contradiction": sort_weight_list(per_class["contradiction"]),
                "neutral": sort_weight_list(per_class["neutral"]),
                "entailment": sort_weight_list(per_class["entailment"]),
    ...
    print(f"[DONE] {spec['display_name']} SHAP run {run_id:02d}: {out_path} | seconds={elapsed:.2f}")
    return out_path


def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Run SHAP explanations for NLI models.")
    ...
    parser.add_argument("--output_dir", type=str, default="./outputs/nli_shap",
    ...
    parser.add_argument("--shap_max_evals", type=int, default=0,
                        help="0 uses SHAP default. Set e.g. 500 or 1000 to cap evaluations.")
...
def main() -> None:
    ...
    summary_records = []
    max_evals = args.shap_max_evals if args.shap_max_evals > 0 else None
    ...
    for model_key in model_keys:
        ...
        for run_id in range(1, args.num_runs + 1):
            out_path = run_shap_for_model_run(
                dx=dx,
            ...
            summary_records.append({"method": "shap", "model": model_key, "run": run_id, "output": str(out_path)})

        del model, tokenizer, predict_proba
        free_memory()

    summary_path = output_dir / "nli_shap_summary_outputs.csv"
    pd.DataFrame(summary_records).to_csv(summary_path, index=False)
    print(f"\nAll SHAP runs completed. Summary saved to: {summary_path}")
