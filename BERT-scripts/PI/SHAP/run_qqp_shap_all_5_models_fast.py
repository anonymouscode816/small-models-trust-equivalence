#!/usr/bin/env python3
"""
Run SHAP explanations for paraphrase identification models.

This version is suitable for anonymized submission:
- no hard-coded usernames or machine-specific paths,
- no private links or tokens,
- all input, checkpoint, and output locations are passed as CLI arguments.

Expected input CSV columns by default:
    question1, question2, is_duplicate

The script writes one SHAP CSV per model per run. These files can be used by
`create_pi_shap_jaccard_table_anonymized.py` to compute Jaccard similarity.
"""
